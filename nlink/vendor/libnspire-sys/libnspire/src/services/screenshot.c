...
/* CX/CX II fix from Vogtinator/libnspire@7d7d962: RLE in 2-pixel units,
 * and transfer using packet_max_datasize instead of a 253-byte cap. */
static void rle_decode(const uint8_t bpp, uint8_t *in, uint8_t *out,
	...
	while (in_size > 1 && out_size) {
		...
		in_size--;
		if (len < 0) {
			len = (-len + 1) * (int)rle_size;
			len = len < (int)out_size ? len : (int)out_size;
			len = len < (int)in_size ? len : (int)in_size;
...
int nspire_screenshot(nspire_handle_t *handle, struct nspire_image **ptr) {
	...
	uint8_t bbp, *tmp = NULL, *tmp_ptr = NULL, *buffer = NULL;
	...
	if ( (ret = data_read(handle, buffer, maxds, NULL)) )
		goto end;

	if ( (ret = data_scan("bwhhhhbb", buffer, maxds,
	...
	rle_decode(bbp, tmp, i->data, in_len, out_len);
	*ptr = i;
	i = NULL;
