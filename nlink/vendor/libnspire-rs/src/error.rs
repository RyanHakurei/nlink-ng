...
pub(crate) fn err(code: c_int) -> Result<()> {
    ...
    // nspire_* returns -NSPIRE_ERR_*. Those values collide with libusb's
    // own negative constants: -8 is NSPIRE_ERR_BUSY, and also
    // LIBUSB_ERROR_OVERFLOW. Match the nspire code first.
    if code < 0 {
        let nspire = code.wrapping_neg() as c_uint;
...
mod tests {
    ...
    #[test]
    fn nspire_busy_is_not_libusb() {
        assert!(matches!(err(0), Ok(())));
        assert!(matches!(err(-1), Err(Error::Timeout)));
        assert!(matches!(err(-4), Err(Error::LibUsb)));
        assert!(matches!(err(-6), Err(Error::InvalidPacket)));
        assert!(matches!(err(-8), Err(Error::Busy)));
    }
}
